...
export default function HeroCarouselSeries({
  series,
  ...
  useEffect(() => {
    const preloadLogo = (item: Series) => {
      ...
      img.onload = () => {
        ...
        setPreloadedLogos(prev => new Set(prev).add(item.logo_path!));
      };
      
      img.onerror = () => {
        console.log(`Error precargando logo: ${item.name}`);
      };
      
      img.src = logoUrl;
    };

    series.forEach(preloadLogo);
  }, [series]);
  ...
  useEffect(() => {
    ...
    if (series[nextIndex]) preloadLogo(series[nextIndex]);
    if (series[prevIndex]) preloadLogo(series[prevIndex]);
    
    if (currentSeries?.logo_path && logoCache.has(currentSeries.logo_path)) {
      setLogoLoaded(true);
      setLogoError(false);
    } else {
      setLogoLoaded(false);
  ...
  }, [currentIndex, series, currentSeries]);

  const preloadLogo = useCallback((item: Series) => {
    if (!item.logo_path || logoCache.has(item.logo_path)) return;
  ...
  const organizeVideosByLanguage = useCallback((item: Series): AudioOption[] => {
  ...
  const getSeriesInfo = useCallback((item: Series): string => {
  ...
  useEffect(() => {
    if (isActive) {
      ...
      trailerTimeoutRef.current = setTimeout(() => {
        const audios = organizeVideosByLanguage(currentSeries);
        if (audios.length > 0) {
  ...
  }, [isActive, currentSeries, organizeVideosByLanguage, trailerDelay]);
  ...
  useEffect(() => {
    if (isAutoPlaying && !isActive) {
      autoPlayRef.current = setInterval(() => {
        setCurrentIndex((p) => (p + 1) % series.length);
        setShowTrailer(false); 
        setTrailerKey(null); 
        setPlayerReady(false);
      }, autoPlayInterval);
  ...
  }, [isAutoPlaying, isActive, series.length, autoPlayInterval]);
  ...
  const backdropUrl = `https://image.tmdb.org/t/p/original${currentSeries.backdrop_path}`;
  const logoUrl = currentSeries.logo_path ? `https://image.tmdb.org/t/p/original${currentSeries.logo_path}` : null;
  
  const year = currentSeries.first_air_date ? new Date(currentSeries.first_air_date).getFullYear() : '';
  const seriesInfo = getSeriesInfo(currentSeries);
  const matchPercent = getMatchPercent(currentSeries.vote_average);
  const genres = currentSeries.genres?.slice(0, 3) || [];
  const isLogoPreloaded = currentSeries?.logo_path && logoCache.has(currentSeries.logo_path);
      ...
      <div className="absolute bottom-0 left-0 w-full pb-20 px-4 sm:px-6 lg:px-12 xl:px-16">
        <div className="max-w-2xl space-y-6">
          ...
          <div className="flex items-center gap-3 text-sm md:text-base text-gray-200 flex-wrap">
            ...
            <span>{year}</span>
            <span className="text-gray-400">•</span>
            <span className="border border-gray-500 px-1.5 py-0.5 text-xs rounded">{currentSeries.ageRating || '13+'}</span>
            <span className="text-gray-400">•</span>
            <span>{seriesInfo}</span>
            <span className="text-gray-400">•</span>
            <span className="text-yellow-500 font-bold">IMDb {currentSeries.vote_average.toFixed(1)}</span>
          ...
          <div className="flex items-center gap-3 pt-2">
            <a 
              href={`https://www.themoviedb.org/tv/${currentSeries.id}`}
              target="_blank"
