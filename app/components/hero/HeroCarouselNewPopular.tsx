...
import { Play, Plus, Volume2, VolumeX, ChevronRight, Info, Globe, ChevronDown, Film, Tv } from 'lucide-react';
...
export default function HeroCarouselNewPopular({
  items,
  ...
  useEffect(() => {
    const preloadLogo = (item: MediaItem) => {
      ...
      img.onerror = () => {
        console.log(`Error precargando logo: ${item.title || item.name}`);
      };
  ...
  useEffect(() => {
    ...
    if (items[nextIndex]) preloadLogo(items[nextIndex]);
    if (items[prevIndex]) preloadLogo(items[prevIndex]);
  ...
  const preloadLogo = useCallback((item: MediaItem) => {
    if (!item.logo_path || logoCache.has(item.logo_path)) return;
  ...
  const organizeVideosByLanguage = useCallback((item: MediaItem): AudioOption[] => {
  ...
  const getSeriesInfo = useCallback((item: MediaItem): string => {
  ...
  const year = isTV 
    ? (currentItem.first_air_date ? new Date(currentItem.first_air_date).getFullYear() : '')
    : (currentItem.release_date ? new Date(currentItem.release_date).getFullYear() : '');
  
  const durationOrSeasons = isTV 
    ? getSeriesInfo(currentItem) 
    : formatRuntime(currentItem.runtime);
      ...
      <div className="absolute bottom-0 left-0 w-full pb-20 px-4 sm:px-6 lg:px-12 xl:px-16">
        <div className="max-w-2xl space-y-6">
          ...
          <div className="flex items-center gap-3 pt-2">
            <a 
              href={`https://www.themoviedb.org/${currentItem.media_type}/${currentItem.id}`}
              target="_blank"
